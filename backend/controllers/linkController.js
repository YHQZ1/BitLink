...
// Create a guest short link (no authentication required)
export const createGuestLink = async (req, res) => {
  try {
    const { originalUrl, customAlias, sessionId } = req.body;

    if (!originalUrl) {
      return res.status(400).json({ error: "Original URL is required" });
    }

    // Validate session ID (guest users get one free link)
    if (!sessionId) {
      return res
        .status(400)
        .json({ error: "Session ID is required for guest links" });
    }

    // Check if guest already has a link in this session
    const existingGuestLinks = await Link.find({
      sessionId,
      user: null,
    });

    if (existingGuestLinks.length >= 1) {
    ...
    // Create the guest link - user will be null for guest links
    const link = new Link({
      originalUrl,
      shortCode: customAlias || undefined, // Let shortid generate if not provided
      sessionId,
      user: null, // This is now allowed since required: false
      isGuestLink: true,
    });

    // The shortUrl will be auto-generated by the pre-save hook
    await link.save();
...
// Migrate guest links to user account after signup
export const migrateGuestLinks = async (req, res) => {
  try {
    ...
    // Find all guest links for this session
    const guestLinks = await Link.find({
      sessionId,
      user: null,
    });
    ...
    if (guestLinks.length === 0) {
      return res.json({
        message: "No guest links to migrate",
        migratedCount: 0,
      });
    }

    // Update all guest links to belong to the new user
    ...
    res.json({
      message: "Guest links migrated successfully",
      migratedCount: updateResult.modifiedCount,
    });
  } catch (error) {
    console.error("Error migrating guest links:", error);
    res.status(500).json({ error: error.message });
  }
};
